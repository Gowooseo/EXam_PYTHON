...
-결과:True,False
-동작방식
         *and=> A and B : A,B 모두 True 일 때만 True
         *or=> A or B : A,B 중 하나 이상 True 되면 True
         *not=> not A: 현재 A의 상태를 반대로 => not True--> False, not False---> True:
         토글(toggle)
...
#          *is not=> A is B : A,B가 서로 다른 데이터 타입의 객체 여부

print(f'10 is 10 => {10 is 10},     10 == 10.0 => {10 == 10.0}')
print(f'10 is 10.0 => {10 is 10.0}')#타입이 int 와 float으로 다름
